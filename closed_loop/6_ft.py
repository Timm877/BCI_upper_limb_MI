...
def execution(test_subject, session):
    ...
    for model_type in ['best_pretrain', 'best_finetune']:
        ...
        next_session = 'session' + str(session_num+1)

        # model path and subjpath for future sessions
        model_path = f"closed_loop/final_models/{model_type}\{session}/EEGNET_{test_subject}"
        savepath_newmodel = f"closed_loop/final_models/models_for_closedloop/EEGNET_{test_subject}_{model_type}_{next_session}"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--session", type=str, default='session0', help="Subject.")
