...
def execution(pipeline_type, list_of_freq_lim, freq_limits_names_list, filt_orders, window_sizes, subject, session):
    ...
    folder_path = Path(f'./scripts/realtime_exp\Expdata\Subjects\wet\{subject}\{session}\openloop')
    result_path = Path(f'./scripts/realtime_exp\intermediate_files/{session}_CSP')
