...
def apply_filter_statespace(sig, A, B, C, D, Xnn):
    ...
    b,a = signal.butter(2, [8/(250/2), 35/(250/2)], 'bandpass')
    filt_sig2 = signal.filtfilt(b, a, sig)

    plt.plot(filt_sig)
    plt.plot(filt_sig2)
    plt.ylim(ymax = 250, ymin = -100)
    plt.title('EEG signal FZ - 10th segment - bandpass filtered 8 to 35Hz')
    plt.xlabel('Datapoints, captured with 250Hz')
...
def pre_processing(curr_segment,selected_electrodes_names ,filters, sample_duration, freq_limits_names, pipeline_type, 
    ...
    # 2 OUTLIER DETECTION --> https://www.mdpi.com/1999-5903/13/5/103/html#B34-futureinternet-13-00103
...
def init_pipelines_grid(pipeline_name = ['csp']):
    ...
    if 'csp' in pipeline_name:
        pipelines["csp_12+s_lda_n"] = Pipeline(steps=[('csp', CSP()), 
