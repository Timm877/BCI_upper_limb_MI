...
def run(test_subject):
    for model_type in ['best_pretrain', 'best_finetune']:
        ...
        model_path = f"final_models/{model_type}\EEGNET_{test_subject}"

        # TODO change testsubj path below
        testsubj_path = Path(f'./data/openloop/intermediate_datafiles/preprocess/Session1/{config.test_subject}_deep.pkl')
        ...
        'val_trial': val_trial,
        'CLsession': 1, #NOTE change num according to session
        'ablation': 'all',
        'seed':  42,    
        'learning_rate': 0.001,
        'filter_sizing':  8,
...
def train(config=None):
    ...
    with wandb.init(project=f"EEGNET_ClosedLoopFT_{config['test_subject']}", config=config):
        config = wandb.config
        ...
        # NOTE save model
        torch.save(net.state_dict(), f"final_models/{config.model_type}\session{config.CLsession}\EEGNET_{config.test_subject}")
